...
| P1 | OTP engine | `crates/totem-otp` | every RFC 4226/6238 vector green, `otpauth` round-trip property test green, fuzz target runs |
| P1 | Crypto core | `crates/totem-crypto` | envelope + backup + recovery-kit round-trips green, KAT vectors green, builds for `wasm32` |
| P2 | Vault | `crates/totem-vault` | CRDT convergence property test green, crash-injection tests green |
| P3 | Interop | `crates/totem-importers` | fixture file per format imports byte-exactly, all fuzz targets run |
| P4 | Sync | `crates/totem-sync`, `server/totem-server` | two simulated clients converge through the real server; hostile-server tests rejected |
| P5 | Bindings | `crates/totem-core`, `crates/totem-ffi` | WASM bundle loads in a browser, UniFFI generates Kotlin + Swift |
...
- **P1 crates are independent** — `totem-otp` has no crypto dependency beyond
  HMAC, and `totem-crypto` knows nothing about OTP. Build both at once.
