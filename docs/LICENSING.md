...
Misty is deliberately not licensed uniformly. GitHub only displays one license per
repository, so this file is the authoritative map.

| Path | License | Why |
|---|---|---|
| `crates/misty-otp/**` | `MIT OR Apache-2.0` | A correct, exhaustively vectored RFC 4226/6238 implementation is worth more to the ecosystem shared than hoarded. Permissive terms let other authenticators adopt it outright, including proprietary ones. |
| `CODE_OF_CONDUCT.md` | `CC-BY-4.0` | Contributor Covenant 2.1, kept verbatim under its own terms. |
| everything else | `AGPL-3.0-or-later` | Anyone may run, audit, fork, and self-host. Anyone offering Misty as a service must publish their changes. |
...
overkill for a desktop app. It is not overkill here: Misty ships a **web app**, and
a web app is conveyed over a network. Someone forking Misty into a hosted
...
`misty-otp` has no product logic in it. It is arithmetic defined by two RFCs, plus
...
ecosystem and gains Misty nothing.
...
```
misty-otp  (MIT OR Apache-2.0)
    │  may be consumed by
    ▼
misty-crypto, misty-vault, misty-sync, apps, server  (AGPL-3.0-or-later)
```

`misty-otp` must therefore never depend on a copyleft crate — including any other
`misty-*` crate. That is not a style preference: a permissive crate with a copyleft
...
## Why `misty-crypto` is not also permissive

The obvious next question. The answer is that `misty-crypto` is not a generic
crypto library — it encodes Misty's envelope format, its key hierarchy, and the
...
- A new file under `crates/misty-otp/` is `MIT OR Apache-2.0`. Do not add a
  dependency there without checking its license.
- A new crate outside `crates/misty-otp/` sets `license.workspace = true`.
  `crates/misty-otp/Cargo.toml` must set `license = "MIT OR Apache-2.0"` explicitly
...
python3 ci/check-otp-permissive.py   # misty-otp's closure stays permissive
